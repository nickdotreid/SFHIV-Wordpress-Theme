<?php

add_action('get_sidebar','sfhiv_post_sidebar',27);
function sfhiv_post_sidebar(){
	if(!is_singular("post")) return;
	dynamic_sidebar("Blog Sidebar");
}

add_action('after_content','sfhiv_post_author',7);
function sfhiv_post_author(){
	if(!is_singular("post")) return;
	$user = get_user_by('login', get_the_author_meta('user_login'));
	if(!$user) return;
	echo '<div class="post-author">';
		include(locate_template('contact-member.php'));
	echo '</div>';
}

add_action('after_content','sfhiv_post_date',8);
function sfhiv_post_date(){
	if(!is_singular("post")) return;
	echo '<div class="post-date">';
	_e('Posted on: ','sfhiv_theme');
	echo get_the_date();
	echo '</div>';
}

?>